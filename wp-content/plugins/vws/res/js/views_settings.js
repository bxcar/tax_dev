...
WPViews.ViewsSettingsScreen = function( $ ) {
	...
	*/
	...
	self.enable_history_management_state = ( $( '#js-wpv-enable-pagination-manage-history' ).length > 0 ) ? $( '#js-wpv-enable-pagination-manage-history' ).prop( 'checked' ) : false;
	
	$( '#js-wpv-enable-pagination-manage-history' ).on( 'change', function() {
		var thiz = $( this ),
		thiz_container = thiz.closest( '.js-wpv-setting-container' ),
		thiz_save_button = thiz_container.find( '.js-wpv-pagination-settings-save' );
		if ( thiz.prop( 'checked' ) == self.enable_history_management_state ) {
			thiz_save_button
				.addClass( 'button-secondary' )
				.removeClass( 'button-primary' )
				.prop( 'disabled', true );
	...
	$( '.js-wpv-pagination-settings-save' ).on( 'click', function( e ) {
		e.preventDefault();
		var thiz = $( this ),
		spinnerContainer = $('<div class="wpv-spinner ajax-loader">').insertBefore( thiz ).show(),
		thiz_container = thiz.closest( '.js-wpv-setting-container' ),
		...
		data = {
			...
			enable_history_management: $( '#js-wpv-enable-pagination-manage-history' ).prop( 'checked' ),
			wpnonce: $('#wpv_pagination_options_nonce').val()
		};
		$.ajax({
            async: false,
            type: "POST",
            ...
            success: function( response ) {
				if ( response.success ) {
					self.enable_history_management_state = $( '#js-wpv-enable-pagination-manage-history' ).prop( 'checked' );
					thiz
						.addClass( 'button-secondary' )
						.removeClass( 'button-primary' )
						.prop( 'disabled', true );
	...
	*/
	...
	$( '.js-wpv-toolset-admin-bar-options' ).on( 'change', function( e ) {
		if ( self.admin_bar_state == $( '#toolset-admin-bar-settings :input' ).serialize() ) {
			$( '.js-wpv-toolset-admin-bar-settings-save' )
				.removeClass( 'button-primary' )
				.addClass( 'button-secondary' )
				.prop( 'disabled', true );
		} else {
	...
	$( '.js-wpv-toolset-admin-bar-settings-save' ).on( 'click', function( e ) {
		e.preventDefault();
		var thiz = $( this ),
		spinnerContainer = $('<div class="wpv-spinner ajax-loader">').insertBefore( thiz ).show(),
		thiz_container = thiz.closest( '.js-wpv-setting-container' ),
		thiz_messages_container = thiz_container.find( '.js-wpv-messages' ),
		data = {
			action: 'wpv_update_toolset_admin_bar_options',
			frontend: $( '#js-wpv-toolset-admin-bar-menu' ).prop( 'checked' ),
			backend: $('.js-wpv-shortcodes-generator:checked').val(),
			wpnonce: $('#wpv_toolset_admin_bar_settings_nonce').val()
		};
		$.ajax({
			async: false,
			type: "POST",
