...
class WP_Views_plugin extends WP_Views {
	...
	*/
    ...
    function admin_menu() {
		...
		add_submenu_page( 'views', __( 'Settings', 'wpv-views' ), __( 'Settings', 'wpv-views' ), $cap, 'views-settings', array( WPV_Settings_Screen::get_instance(), 'wpv_settings_admin' ) );
		...
		foreach( $used_view_ids as $view_id ) {
			...
			if( null != $view && $view->is_a_view() ) {
				...
				if( !$is_disabled && !empty( $url ) ) {
					...
					$menu_item_definitions[] = array(
						'title' => sprintf( '%s: %s', __( 'Edit View', 'wp-views' ), $view->title ),
		...
		$layout_wizard_translations = array(
            ...
            'bootstrap_not_set' => __( 'You need to set the Bootstrap version used in your theme.', 'wpv-views' ) . ' ' .
                sprintf(
                    ...
                    esc_url( add_query_arg( array( 'page' => 'views-settings', 'tab' => 'compatibility' ), admin_url( 'admin.php' ) ) )
                ),
