...
class WPV_Settings_Screen {
     ...
     * Views Settings page set up
 	 *
	 * @todo this can be optimized heavily - hooks and assets only when needed.
     */
    function init() {
		...
		add_action( 'wpv_action_views_settings_features_section', array( $this, 'wpv_admin_bar_options' ), 50 );
		add_action( 'wpv_action_views_settings_features_section', array( $this, 'wpv_codemirror_options' ), 60 );
		add_action( 'wpv_action_views_settings_features_section', array( $this, 'wpv_pagination_options' ), 70 );
		...
		add_action( 'wp_ajax_wpv_update_toolset_admin_bar_options', array( $this, 'wpv_update_toolset_admin_bar_options' ) );
		...
		add_filter( 'wpv_filter_wpv_enable_pagination_manage_history', array( $this, 'wpv_filter_wpv_enable_pagination_manage_history' ), 1 );
    }
    ...
    function wpv_settings_admin() {
        ...
        <div class="wrap">
            ...
            <div class="wpv-settings-tab-content">
                <?php do_action( "wpv_action_views_settings_{$tab}_section", $settings ); ?>
            </div>

            <?php do_action( "wpv_action_views_settings_sections", $settings ); ?>
    ...
    function wpv_show_hidden_custom_fields_options( $options ) {
        ...
        if ( isset( $settings->wpv_show_hidden_fields ) && $settings->wpv_show_hidden_fields != '' ) {
	...
	function wpv_admin_bar_options( $options ) {
		...
		<div class="wpv-setting-container js-wpv-setting-container">
			<div class="wpv-settings-header">
				<h2><?php _e( 'Admin bar options', 'wpv-views' ); ?></h2>
			</div>
			<div id="toolset-admin-bar-settings" class="wpv-setting">
				<h3><?php echo __( 'Toolset shortcodes menu in the admin bar', 'wpv-views' ); ?></h3>
					...
					<ul class="js-shortcode-generator-form">
						<?php
							...
							foreach( $shortcodes_generator_options as $option ) {
								printf(
									'<li><label><input type="radio" name="wpv-shortcodes-generator" class="js-wpv-shortcodes-generator js-wpv-toolset-admin-bar-options" value="%s" %s autocomplete="off" />%s</label></li>',
									$option['value'],
				...
				<h3><?php echo __( 'Design with Toolset', 'wpv-views' ); ?></h3>
				<div class="wpv-advanced-setting">
					<p>
					...
					<p>
						<label>
							<input type="checkbox" name="wpv-toolset-admin-bar-menu" id="js-wpv-toolset-admin-bar-menu" class="js-wpv-toolset-admin-bar-menu js-wpv-toolset-admin-bar-options" value="1" <?php checked( $toolset_admin_bar_menu_show ); ?> autocomplete="off" />
							<?php _e( "Enable the Design with Toolset menu in the frontend", 'wpv-views' ); ?>
						</label>
					</p>
				</div>
				<?php
				wp_nonce_field( 'wpv_toolset_admin_bar_settings_nonce', 'wpv_toolset_admin_bar_settings_nonce' );
				?>
				<p class="update-button-wrap">
					<span class="js-wpv-messages"></span>
					<button class="js-wpv-toolset-admin-bar-settings-save button-secondary" disabled="disabled">
						<?php _e( 'Save', 'wpv-views' ); ?>
					</button>
				</p>
	...
	function wpv_pagination_options( $options ) {
		$settings = WPV_Settings::get_instance();
        ?>

        <div class="wpv-setting-container js-wpv-setting-container">
            <div class="wpv-settings-header">
                <h2><?php _e( 'Pagination options', 'wpv-views' ); ?></h2>
            </div>
            <div class="wpv-setting">
				<p>
                    <?php _e( "You can adjust some global settings related to the Views pagination here.", 'wpv-views' ); ?>
                </p>
                <ul class="">
                    <li>
						<h3><?php _e( 'Browser history management', 'wpv-views' ); ?></h3>
						<p>
                        ...
                        <label>
                            <input id="js-wpv-enable-pagination-manage-history" type="checkbox" name="wpv-enable-pagination-manage-history" class="js-wpv-enable-pagination-manage-history" value="on" <?php checked( $settings->wpv_enable_pagination_manage_history == 1 ); ?> autocomplete="off" />
                ...
                <?php
                wp_nonce_field( 'wpv_pagination_options_nonce', 'wpv_pagination_options_nonce' );
                ?>
                <p class="update-button-wrap">
                    <span class="js-wpv-messages"></span>
                    <button class="js-wpv-pagination-settings-save button-secondary" disabled="disabled">
                        <?php _e( 'Save', 'wpv-views' ); ?>
                    </button>
                </p>
            </div>
            ...
            <div class="wpv-setting">
                <div class="js-debug-settings-form">
					...
					echo '<p>'
						...
						. '</p>';
					?>
					<input type="text" id="wpv-debug-theme-function" class="js-wpv-debug-theme-function" name="wpv_theme_function" value="<?php echo $settings->wpv_theme_function; ?>" autocomplete="off" />
					...
					<p>
						<input type="checkbox" id="wpv-debug-theme-function-enable-debug" class="js-wpv-debug-theme-function-enable-debug" name="wpv_theme_function_debug" value="1" <?php checked( $settings->wpv_theme_function_debug ); ?> autocomplete="off" />
        ...
        <div class="wpv-setting-container js-wpv-setting-container">
            ...
            <div class="wpv-setting">
                ...
                <div class="js-debug-mode-form">
                    ...
                    <div class="wpv-advanced-setting js-wpv-debug-additional-options<?php echo empty( $settings->wpv_debug_mode ) ? ' hidden' : ''; ?>">
                        <ul>
                            <li><label>
                        ...
                        <?php
                        $debug_enabled = ( isset( $settings->wpv_debug_mode ) && $settings->wpv_debug_mode );
                        $debug_dismissed = ( isset( $settings->dismiss_debug_check ) && $settings->dismiss_debug_check == 'true' );
	...
	*/
    ...
    function wpv_get_show_hidden_custom_fields() {
		...
		if (
			...
			$settings->wpv_show_hidden_fields = isset( $_POST['wpv_show_hidden_fields'] ) ? implode( ',', $_POST['wpv_show_hidden_fields'] ) : '';
			$settings->save();
	...
	function wpv_update_toolset_admin_bar_options() {
		if ( ! current_user_can( 'manage_options' ) ) {
			$data = array(
				'type' => 'capability',
				'message' => __( 'You do not have permissions for that.', 'wpv-views' )
		...
		if ( 
			! isset( $_POST["wpnonce"] )
			|| ! wp_verify_nonce( $_POST["wpnonce"], 'wpv_toolset_admin_bar_settings_nonce' ) 
		) {
			$data = array(
				'type' => 'nonce',
				'message' => __( 'Your security credentials have expired. Please reload the page to get new ones.', 'wpv-views' )
	...
	function wpv_update_pagination_options() {
		...
		if ( 
			! isset( $_POST["wpnonce"] )
			|| ! wp_verify_nonce( $_POST["wpnonce"], 'wpv_pagination_options_nonce' ) 
		) {
			$data = array(
				'type' => 'nonce',
				'message' => __( 'Your security credentials have expired. Please reload the page to get new ones.', 'wpv-views' )
		...
		$enable_history_management = ( isset( $_POST['enable_history_management'] ) ) ? sanitize_text_field( $_POST['enable_history_management'] ) : '';
		$settings->wpv_enable_pagination_manage_history = ( $enable_history_management == 'true' ) ? 1 : 0;
		$settings->save();
		wp_send_json_success();
	}
	...
	*/
    ...
    function wpv_switch_debug_check() {
		...
		if ( ! is_null( $result ) ) {
			$settings->dismiss_debug_check = ( $result == 'recover' ) ? /* recover */ 'false' : /* dismiss */ 'true';
            $settings->save();
			wp_send_json_success();
		} else {
			$data = array(
	...
	function wpv_filter_wpv_enable_pagination_manage_history( $status ) {
		$settings = WPV_Settings::get_instance();
